...
(`tools/surface.sh check`). `use glfw.glfw;` binds the leaf as `glfw` and
gives the whole API as `glfw.init()`, `glfw.create_window(...)`,
`glfw.KEY_ESCAPE`. The split modules (`glfw.core`, `glfw.window`, …) remain
importable individually for smaller dependency surfaces.
